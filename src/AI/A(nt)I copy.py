...
class AIPlayer(Player):
    ...
    def getMove(self, currentState):
        ...
        for drone in myDrones:
            if not (drone.hasMoved):
                if drone.coords == self.myFood.coords:
                    return Move(MOVE_ANT, [drone.coords, (drone.coords[0], drone.coords[1]+1)], None)
                elif drone.coords == self.myTunnel.coords:
                    return Move(MOVE_ANT, [drone.coords, (drone.coords[0], drone.coords[1]+1)], None)
                elif drone.coords == myInv.getAnthill().coords:
                    return Move(MOVE_ANT, [drone.coords, (drone.coords[0], drone.coords[1]+1)], None)
